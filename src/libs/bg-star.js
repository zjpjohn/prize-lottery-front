...
let BgStar = function (w, h, maxStars) {
...
BgStar.prototype.draw = function (ctx, canvas) {
  ...
  ctx.drawImage(canvas, x - this.radius / 2, y - this.radius / 2, this.radius, this.radius);
  this.timePassed += this.speed;
};

export function initializeBg(canvasId) {
...
// draw a big beefy gradient in the center of the dummy canvas
  ...
  for (var i = 0; i < maxStars; i++) {
    stars[i] = new BgStar(w, h, maxStars);
  }
