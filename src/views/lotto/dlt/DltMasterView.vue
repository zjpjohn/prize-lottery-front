...
import {dlt_channels} from "../../../libs/lottery";
import {lastDltPeriod} from "../../../libs/period";
  ...
  methods: {
    ...
    rankPeriod() {
      latestRank().then(period => {
        if (period) {
          for (let i = 0; i < 10; i++) {
            this.periods.push(lastDltPeriod(period, i));
          }
        }
      })
    },
