...
import {fsdItemCensus, itemRankList, mulRankList} from "../../../api/fc3d-service";
import {d3_categories, fsd_channels, parseForecast} from "../../../libs/lottery";
...
export default {
  ...
  data() {
    return {
      showChart: false,
      channels: fsd_channels.filter(e => e.vip),
      channel: 'd3',
      period: null,
      showPoster: false,
      selected: null,
  ...
  methods: {
    ...
    queryItemCensus() {
      ...
      fsdItemCensus(this.period).then(data => this.census = data);
    },
    toItemTable() {
      this.$router.push('/fsd/data/item/table');
    },
    toWholeBest() {
      this.$router.push('/fsd/data/item/whole')
    },
    showItemCensus() {
    ...
    downloadPoster() {
      ...
      let name = `fc3d_${this.selected.period}`;
...
<style scoped>
::v-deep .el-table__header-wrapper th.currentColumn {
  color: #3a8ee6;
}
</style>