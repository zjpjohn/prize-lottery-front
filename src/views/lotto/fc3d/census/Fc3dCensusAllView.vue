...
import {calcFc3dAllChart, getFc3dFullChart} from "../../../../api/fc3d-service";
...
export default {
  ...
  methods: {
    calcFullChart() {
      calcFc3dAllChart(this.period).then(res => {
        this.cancelCalcFull();
        this.queryFullChart();
      })
    },
    ...
    queryFullChart() {
      getFc3dFullChart(this.query).then(data => {
        let {
              period,
              level100,
              level200,
        ...
        if (this.duplex.indexOf(this.query.type) >= 0) {
          ...
          for (let i = 0; i < d3_categories.length; i++) {
            ...
            dataShi.push({
              ...
              '前400': level400.values[1][i],
              '前600': level600.values[1][i],
              '前800': level800.values[1][i],
              '全部': level1000.values[1][i],
            });
            dataGe.push({
              '号码': d3_categories[i],
              '前100': level100.values[2][i],
              '前200': level200.values[2][i],
              '前400': level400.values[2][i],
              '前600': level600.values[2][i],
              '前800': level800.values[2][i],
              '全部': level1000.values[2][i],
            });
          }
