<template>
  <div style="margin: 32px 32px 0 32px;">
    <el-form :inline="true" :model="query" :rules="rules" ref="queryForm" label-with="90px">
      ...
      <el-form-item label="第一胆码" prop="best">
        <el-select
            v-model="query.best"
            style="width: 200px"
            clearable
          ...
          <el-option
              ...
              :key="item.value"
              :label="item.label"
              :disabled="disableBest(item.value)"
              :value="item.value">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="第二胆码" prop="secondary">
        <el-select
            v-model="query.secondary"
            style="width: 200px"
            clearable
            placeholder="请选择第二胆码">
          <el-option
              ...
              :key="item.value"
              :label="item.label"
              :disabled="disableSecondary(item.value)"
              :value="item.value">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="优质选号" prop="mores">
        <el-select
            v-model="query.mores"
            multiple
            collapse-tags
            ...
            style="width: 200px"
            placeholder="请选择更多优质号码">
          <el-option
              v-for="item in numList"
              :key="item.value"
              :label="item.label"
              :disabled="disableMores(item.value)"
              :value="item.value">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-button plain type="primary" @click="fc3dFilterQuery">过滤计算</el-button>
      </el-form-item>
      <el-form-item>
        <el-button plain type="primary" @click="savePivot">保存结果</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-dialog :visible.sync="showPoster" width="418px" @before-close="cancelPoster">
      <div slot="title">福彩3D预测海报</div>
      <div style="text-align: center">
        <div ref="fc3d_forecast"
             v-if="selected">
          <div
            ...
            <div style="font-size: 22px;font-weight:500;margin:0 0 6px 32px">福彩3D</div>
...
import {addFc3dPivot, fc3dDanFilter} from "../../../../api/fc3d-service";

export default {
  name: "Fc3dBestDataView",
  ...
  methods: {
    disableBest(value) {
      return this.query.secondary === value || this.query.mores.includes(value);
    },
    disableSecondary(value) {
      return this.query.best === value || this.query.mores.includes(value);
    },
    disableMores(value) {
      return this.query.best === value || this.query.secondary === value;
    },
    forecastItems(data) {
      if (data.hitData) {
    ...
    fc3dFilterQuery() {
      this.$refs.queryForm.validate(valid => {
        ...
        this.list = [];
        fc3dDanFilter(this.query).then(data => this.list = data || []);
      });
    },
    savePivot() {
      if (this.list.length === 0) {
        this.$message.error('请先计算结果');
        return;
      }
      this.$refs.queryForm.validate(valid => {
        if (!valid) {
        ...
        this.$confirm('是否要保存到今日要点？', '重要提示',
          ...
          addFc3dPivot({
            ...
            best: this.query.best,
            second: this.query.secondary,
            quality: this.query.mores,
    ...
    downloadPoster() {
      let width               = this.$refs.fc3d_forecast.style.width;
      let cloneDom            = this.$refs.fc3d_forecast.cloneNode(true);
      cloneDom.style.position = 'absolute';
      cloneDom.style.top      = '0px';
      cloneDom.style.zIndex   = '-1';
      cloneDom.style.width    = width;
      document.body.appendChild(cloneDom);
      let name = `fc3d_best_${this.selected.period}`;
