...
<script>
import {calcFc3dPivotHit, fsdPivotList} from "../../../../api/fc3d-service";

export default {
  name: "Fc3dPivotListView",
  data() {
    return {
      page: 1,
      limit: 10,
  ...
  methods: {
    queryPivotList() {
      ...
      fsdPivotList(params).then(data => this.data = data);
    },
    routeToDetail(data) {
      this.$router.push(`/fsd/data/pivot/detail/${data.id}/${data.period}`);
    },
    routeToFilter() {
      this.$router.push('/fsd/data/pivot/filter');
    },
    calcPivotHit(period) {
      this.$confirm('是否手动计算命中信息？', '重要提示',
        ...
        calcFc3dPivotHit(period).then(res => {
          this.queryPivotList();
