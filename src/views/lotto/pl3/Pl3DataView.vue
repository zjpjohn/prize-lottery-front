  ...
  <div style="margin: 32px 32px 0 32px">
    <permission permit="super">
      ...
      <div style="margin: 24px 0;display: flex;align-items: center">
        ...
        <el-button size="medium" icon="el-icon-refresh-left" @click="fetchForecast">同步数据</el-button>
        <el-button size="medium" icon="el-icon-s-operation" @click="calcHit">命中计算</el-button>
...
import {limitName, pls_channels} from "../../../libs/lottery";
  ...
  methods: {
    ...
    fetchForecast() {
      if (this.period == null || this.period === '') {
        this.$message.success('请输入同步期号')
        return;
      }
      this.$confirm('是否手动同步预测数据？', '重要提示',
          {
    ...
    calcHit() {
      this.$confirm('是否手动计算预测命中信息？', '重要提示',
          {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
    ...
    toItemForecast() {
      this.$router.push('/pls/data/item');
    },
    toDanFilter() {
      this.$router.push('/pls/data/dan/filter');
    },
    toTodayPivot() {
      this.$router.push('/pls/data/pivot');
    },
  },
  mounted() {
