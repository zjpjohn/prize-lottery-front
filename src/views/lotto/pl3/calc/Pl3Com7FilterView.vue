...
import {com7Combine, latestPeriod} from "../../../../api/pl3-service";

export default {
  name: "Pl3Com7FilterView",
  data() {
    return {
      periods: [],
      filterDto: {
  ...
  methods: {
    ...
    queryPeriods() {
      latestPeriod().then(data => {
        ...
        for (let i = 0; i < 90; i++) {
          this.periods.push(lastPlsPeriod(period, i));
        }
    ...
    queryLottery() {
      num3LastLottery('pl3', this.period).then(data => {
        this.lottery = {
          period: data.period,
